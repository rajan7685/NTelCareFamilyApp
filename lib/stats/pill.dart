...
//final List<charts.Series<HeartStatWeekMax, String>> seriesList = _createSampleData();
  ...
  void getStepMonthlyRate() async {
    ...
    print(wDate);
    var response = await getHrate.get(
        'http://18.208.148.208:4000/graph/health_status/PillBox/monthly?date=${wDate}&senior_id=${id}');
    print(response.statusCode);
    print(response.body.runtimeType);
    print(response.body);
    final rate = jsonDecode((response.body));
  ...
  void getStepYearlyRate() async {
    ...
    print(wDate2);
    var response = await getHrate.get(
        'http://18.208.148.208:4000/graph/health_status/PillBox/yearly?date=${wDate}&senior_id=${id}');
    print(response.statusCode);
    print(response.body.runtimeType);
    print(response.body);
    final rate = jsonDecode((response.body));
...
List<PillboxStatMax> PillboxStatWeekMinFromJson(List<dynamic> rate) {
  ...
  rate.forEach((element) {
    heart
        .add(PillboxStatMax(element["date"], element["min"], null, null, null));
  });
  return heart;
}

List<PillboxStatMax> PillboxStatWeekFromJson(List<dynamic> rate) {
  List<PillboxStatMax> heart = [];

  rate.forEach((element) {
    heart.add(PillboxStatMax(element["date"], element["value"], element["avg"],
