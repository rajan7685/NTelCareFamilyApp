...
class BatteryWidget extends StatefulWidget {
  String data;
  BatteryWidget({Key key, @required this.data}) : super(key: key);

  @override
  _BatteryWidgetState createState() => _BatteryWidgetState(data);
}
...
class _BatteryWidgetState extends State<BatteryWidget> {
  ...
  List<batteryStatMax> stepMaxMon = [];
  List<batteryStatMax> stepMinMon = [];
  List<batteryStatMax> stepMinYr = [];
  List<batteryStatMax> stepMaxYr = [];
...
*/
  ...
  List<charts.Series<batteryStatMax, String>> _createSampleData() {
    return [
      new charts.Series<batteryStatMax, String>(
          id: 'max2',
          domainFn: (batteryStatMax sales, _) => sales.day,
          measureFn: (batteryStatMax sales, _) => sales.Stat,
          data: stepMax,
          colorFn: (_, __) => charts.Color.fromHex(code: "#00B89F"),
          labelAccessorFn: (batteryStatMax sales, _) =>
              '\$${sales.Stat.toString()}'),
      new charts.Series<batteryStatMax, String>(
          id: 'min2',
          domainFn: (batteryStatMax sales, _) => sales.day,
  ...
  List<charts.Series<batteryStatMax, String>> _createSampleDataMonth() {
    return [
      new charts.Series<batteryStatMax, String>(
          id: 'max3',
          domainFn: (batteryStatMax sales, _) => sales.day,
  ...
  List<charts.Series<batteryStatMax, String>> _createSampleDataYear() {
    return [
      new charts.Series<batteryStatMax, String>(
          id: 'max9',
          domainFn: (batteryStatMax sales, _) => sales.day,
...
//final List<charts.Series<HeartStatWeekMax, String>> seriesList = _createSampleData();
  ...
  void getHeartRate() async {
    ...
    print(response.body);
    final rate = jsonDecode((response.body));
    print(rate["health_status"][0]["battery"].runtimeType);

    List<batteryStat> temp =
        batteryStatFromJson(rate["health_status"][0]["battery"]);
    setState(() {
      hRate = temp;
    });
    print(hRate);
  ...
  void getBatteryWeekRate() async {
    ...
    List<batteryStatMax> temp = batteryStatWeekMaxFromJson(rate["battery"]);
    setState(() {
      stepMax = temp;
    });
    List<batteryStatMax> temp1 = batteryStatWeekMinFromJson(rate["battery"]);
    setState(() {
      stepMin = temp1;
    });
  }
  ...
  void getBatteryMonthlyRate() async {
    wDate1 = DateFormat('yyyy-MM-dd').format(dateTimeMonth);
    print(wDate);
    var response = await getHrate.get(
        'http://18.208.148.208:4000/graph/health_status/battery/monthly?date=${wDate}&senior_id=${id}');
    print(response.statusCode);
    print(response.body.runtimeType);
    print(response.body);
    final rate = jsonDecode((response.body));
  ...
  void getBatteryYearlyRate() async {
    wDate2 = DateFormat('yyyy-MM-dd').format(dateTimeMonth);
    print(wDate2);
    var response = await getHrate.get(
        'http://18.208.148.208:4000/graph/health_status/battery/yearly?date=${wDate}&senior_id=${id}');
    print(response.statusCode);
    print(response.body.runtimeType);
    print(response.body);
    final rate = jsonDecode((response.body));
...
List<batteryStatMax> batteryStatWeekMinFromJson(List<dynamic> rate) {
  List<batteryStatMax> heart = [];

  rate.forEach((element) {
    heart.add(batteryStatMax(element["date"], element["min"], element["avg"],
        element["max"], element["min"]));
  });
  return heart;
}

List<batteryStatMax> batteryStatWeekMaxFromJson(List<dynamic> rate) {
  List<batteryStatMax> heart = [];

  rate.forEach((element) {
    heart.add(batteryStatMax(element["date"], element["max_min"],
        element["avg"], element["max"], element["min"]));
  });
  return heart;
}

class batteryStatMax {
  final String day;
  final int Stat;
  final int avg;
  final int max;
  ...
  batteryStatMax(this.day, this.Stat, this.avg, this.max, this.min);
}

List<batteryStat> batteryStatFromJson(List<dynamic> rate) {
  List<batteryStat> step = [];
  rate.forEach((element) {
    step.add(batteryStat(time: element["time"], value: element["value"]));
  });
  return step;
}
// List<HeartStat>.from(json.decode(str).map((x) => HeartStat.fromJson(x)));

String batteryStatToJson(List<batteryStat> data) =>
